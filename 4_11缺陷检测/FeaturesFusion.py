...
class AnomalyDetector:
    ...
    def train(self, features_list):
        # 使用一类SVM进行训练
        self.model = OneClassSVM(gamma='auto').fit(features_list)
...
if __name__ == "__main__":
    ...
    # 示例训练数据集
    # 注意：这里为了简化示例，直接使用相同的图像。实际应用中应使用不同的正常样本进行训练。
    features_list = [fused_feature for _ in range(10)]

    # 检查所有特征向量的形状是否一致
    feature_length = fused_feature.shape[0]
    for feature in features_list:
        assert feature.shape[0] == feature_length, "特征向量长度不一致！"
    ...
    # 检查新特征向量的长度是否与训练特征向量的长度一致
    if new_fused_feature.shape[0] != feature_length:
        raise ValueError(f"新特征向量长度 {new_fused_feature.shape[0]} 与训练特征长度 {feature_length} 不一致。")
